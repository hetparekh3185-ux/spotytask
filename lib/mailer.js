/**
 * SPOTITASK — Email Dispatch Service (lib/mailer.js)
 * Supports both Resend API and Direct Gmail SMTP with Spotify-themed HTML alerts.
 */
...
function buildHtmlTemplate({ taskTitle, taskNotes, taskCategory, taskDue, emoji, appUrl }) {
  const actionHtml = `
    <!-- CTA -->
    <tr>
...
async function sendTaskEmail({ to, subject, message, task }) {
  ...
  const taskDue = (task && (task.dueDatetime || task.dueTime)) 
    ? new Date(task.dueDatetime || task.dueTime).toLocaleString() 
    : 'Now';

  const emoji = CATEGORY_EMOJIS[taskCategory] || '⏰';
  const cleanSubject = subject || `⏰ SpotiTask: "${taskTitle}" is due now!`;
  const appUrl = process.env.APP_URL || 'https://spotytask.vercel.app';

  const htmlBody = buildHtmlTemplate({
    taskTitle,
    taskNotes,
    taskCategory,
    taskDue,
  ...
  if (hasGmailSmtp) {
    try {
      ...
      console.log(`[SendEmail] Sent via Gmail SMTP to ${to}:`, info.messageId);
      return {
        success: true,
        statusCode: 200,
        id: info.messageId,
      ...
      console.error('[SendEmail] Gmail SMTP error, checking Resend fallback:', smtpErr);
      if (!hasResend) {
        return {
          success: false,
          statusCode: 500,
          error: smtpErr.message || 'Gmail SMTP dispatch failed',
          hint: 'Ensure your GMAIL_USER is correct and GMAIL_APP_PASSWORD is a valid 16-character Google App Password (not your personal Gmail password).'
        };
      }
    }
  }
  ...
  if (hasResend) {
    try {
      ...
      if (result.error) {
        ...
        const isDomainRestriction = result.error.statusCode === 403 || 
          (result.error.message && result.error.message.includes('only send testing emails'));
        return {
          success: false,
          ...
          hint: isDomainRestriction
            ? 'Free tier only allows sending to your Resend registered account email. To send to any recipient, verify your domain at resend.com/domains, or use direct Gmail SMTP.'
            : 'Check the recipient email address and your Resend dashboard logs.'
        };
      }

      console.log(`[SendEmail] Dispatched via Resend to ${to}. ID: ${result.data?.id}`);
      return {
        success: true,
        statusCode: 200,
        id: result.data?.id,
        provider: 'resend',
        message: `Email accepted by Resend API for ${to}.`
      };
    } catch (err) {
  ...
  return {
    success: false,
    statusCode: 500,
    error: 'No valid email transport available'
  };
}
